...
/**
 * Escape a value that came from a user before interpolating it into email HTML.
 * Names and team strings reach templates from `anonymous_picks` / `picks`, both
 * of which anyone can write, and the confirmation can be addressed to someone
 * other than whoever typed the name — so an unescaped name is a way to deliver
 * arbitrary markup to a third party over our sending domain.
 */
