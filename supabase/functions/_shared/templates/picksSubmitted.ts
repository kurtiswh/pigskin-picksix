...
export function getPicksSubmittedHtml(data: PicksSubmittedData): string {
  const picksHtml = data.picks.map(pick => {
    ...
    return `<tr style="border-bottom:1px solid ${EMAIL.line}">
      <td style="padding:10px 8px;color:${EMAIL.ink};font-weight:${w}">${pick.isLock ? '🔒 ' : ''}${esc(pick.game)}</td>
      <td style="padding:10px 8px;color:${EMAIL.ink};text-align:center;font-weight:${w}">${esc(pick.pick)} (${spreadText})</td>
    </tr>`
  }).join('')
  ...
  return emailShell({
    ...
    heading: `Hi ${esc(data.userDisplayName)}!`,
