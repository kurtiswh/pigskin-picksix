...
-- Add unique partial index to ensure only one lock per user per week instead of complex EXCLUDE constraint
CREATE UNIQUE INDEX idx_anonymous_picks_one_lock_per_user_week 
ON public.anonymous_picks (email, week, season) 
WHERE is_lock = true;
