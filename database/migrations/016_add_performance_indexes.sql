...
-- Games table indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_games_week_season 
ON public.games (week, season);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_games_status 
ON public.games (status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_games_kickoff_time 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_picks_game_id 
ON public.picks (game_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_picks_week_season 
ON public.picks (week, season);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_picks_user_week_season 
ON public.picks (user_id, week, season);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_picks_season 
ON public.picks (season);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_picks_result 
ON public.picks (result);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_picks_submitted 
ON public.picks (submitted);

-- Week settings indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_week_settings_week_season 
ON public.week_settings (week, season);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_week_settings_picks_open 
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'leaguesafe_payments') THEN
        CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_leaguesafe_payments_season 
        ON public.leaguesafe_payments (season);
        
        CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_leaguesafe_payments_status 
        ON public.leaguesafe_payments (status);
        
        CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_leaguesafe_payments_user_id 
        ON public.leaguesafe_payments (user_id);
        
        CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_leaguesafe_payments_email 
        ON public.leaguesafe_payments (leaguesafe_email);
        
        CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_leaguesafe_payments_matched 
        ON public.leaguesafe_payments (is_matched);
    END IF;
END $$;

-- Composite indexes for common query patterns
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_picks_season_status_user 
ON public.picks (season, result, user_id) WHERE result IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_picks_lock_result 
