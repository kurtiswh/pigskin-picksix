-- Migration 127: Remove last_updated column references from leaderboard functions
-- 
-- PURPOSE: Fix the "column last_updated does not exist" error by removing
-- references to the last_updated column in leaderboard functions.

DO $$
BEGIN
    RAISE NOTICE '🔧 Migration 127: Remove last_updated column references';
    RAISE NOTICE '===================================================';
END;
$$;

-- Fix the season leaderboard function to remove last_updated column
...
BEGIN
    ...
    -- Calculate combined stats directly (without last_updated)
...
-- Fix the weekly leaderboard function similarly (remove last_updated)
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '✅ Migration 127 COMPLETED - Removed last_updated column references!';
    RAISE NOTICE '';
    RAISE NOTICE '🔧 FIXED ISSUES:';
    RAISE NOTICE '• Removed last_updated from INSERT statements';
    RAISE NOTICE '• Removed last_updated from ON CONFLICT DO UPDATE';
    RAISE NOTICE '• Functions now work with existing table schema';
    RAISE NOTICE '';
