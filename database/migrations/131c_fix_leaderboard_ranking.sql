-- Migration 131c: Fix Leaderboard Ranking to Sort by Points
-- 
-- PURPOSE: Correct the ranking logic to properly sort by total_points for both weekly and season views
-- The UNION ALL approach was creating separate rankings - this fixes it with proper CTEs

DO $$
BEGIN
    RAISE NOTICE '🔧 Migration 131c: Fixing leaderboard ranking to properly sort by points';
    RAISE NOTICE '===============================================================';
END;
$$;
...
-- Step 2: Create weekly_leaderboard with proper ranking
...
-- Step 3: Create season_leaderboard with proper ranking
...
DO $$
BEGIN
    RAISE NOTICE '✅ Migration 131c completed successfully';
    RAISE NOTICE '📊 Leaderboard views now properly rank by total_points';
    RAISE NOTICE '✓ Weekly leaderboard: Ranked by points DESC, then wins DESC';
    RAISE NOTICE '✓ Season leaderboard: Ranked by points DESC, then total_wins DESC';
    RAISE NOTICE '✓ Both views use CTEs for proper unified ranking';
    RAISE NOTICE '✓ Added performance indexes for faster queries';
END;
$$;