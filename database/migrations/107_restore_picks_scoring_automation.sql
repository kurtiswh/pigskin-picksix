-- Migration 107: Restore Picks Scoring Automation After Game Completion
-- 
-- ISSUE: Pick scoring automation broken after Migration 106 disabled triggers
-- USER EXPECTATION: "update games to completed status, base_points and margin_bonus, then triggers updating picks table & anonymous_picks scoring"
-- SOLUTION: Ensure the completion trigger chain works: game completion → game scoring → pick scoring

DO $$
BEGIN
    RAISE NOTICE '🔧 Migration 107: RESTORING PICKS SCORING AUTOMATION';
    RAISE NOTICE '==================================================';
    RAISE NOTICE 'ISSUE: Pick scoring automation disrupted by Migration 106';
    RAISE NOTICE 'USER EXPECTATION: Games complete → Game scoring → Pick scoring';
    RAISE NOTICE 'SOLUTION: Verify and restore completion trigger chain';
    RAISE NOTICE '';
END;
$$;

-- Step 1: Verify the game completion trigger exists and is active
DO $$
DECLARE
    trigger_exists BOOLEAN;
BEGIN
    RAISE NOTICE '📋 Checking game completion triggers...';
    
    SELECT EXISTS (
        SELECT 1 FROM information_schema.triggers 
        WHERE trigger_name = 'handle_game_completion_only_trigger' 
        AND event_object_table = 'games'
    ) INTO trigger_exists;
    
    IF trigger_exists THEN
        RAISE NOTICE '✅ handle_game_completion_only_trigger EXISTS';
    ELSE
        RAISE NOTICE '❌ handle_game_completion_only_trigger MISSING - will recreate';
    END IF;
    
    SELECT EXISTS (
        SELECT 1 FROM information_schema.triggers 
        WHERE trigger_name = 'process_picks_on_completion_trigger' 
        AND event_object_table = 'games'
    ) INTO trigger_exists;
    
    IF trigger_exists THEN
        RAISE NOTICE '✅ process_picks_on_completion_trigger EXISTS';
    ELSE
        RAISE NOTICE '❌ process_picks_on_completion_trigger MISSING - will recreate';
    END IF;
END $$;

-- Step 2: Ensure calculate_pick_results function exists and works
DO $$
DECLARE
...
BEGIN
    ...
    IF function_exists THEN
        RAISE NOTICE '✅ calculate_pick_results() function EXISTS';
    ELSE
        RAISE NOTICE '❌ calculate_pick_results() function MISSING - will recreate';
    END IF;
END $$;

-- Step 3: Recreate the calculate_pick_results function if needed (from Migration 102)
...
-- Step 4: Recreate the completion-only game scoring trigger (from Migration 093)
...
-- Step 5: Recreate the pick processing trigger (from Migration 098)
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RAISE WARNING '❌ Error in pick processing trigger for game %: %', NEW.id, SQLERRM;
        RETURN NEW;
END;
$$;

-- Step 6: Recreate the triggers on games table
...
-- Step 7: Test the automation with a mock scenario (if there are any non-completed games with scores)
DO $$
DECLARE
    test_game_id UUID;
    test_game RECORD;
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '🧪 TESTING AUTOMATION...';
    ...
    IF test_game_id IS NOT NULL THEN
        ...
        RAISE NOTICE '   Current status: % → Will test changing to completed', test_game.status;
        
        -- This should trigger both triggers and process picks automatically
        UPDATE public.games 
    ...
    ELSE
        ...
        RAISE NOTICE '💡 Automation will work when liveUpdateService marks games as completed';
    END IF;
END $$;
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '✅ Migration 107 COMPLETED - Picks scoring automation restored!';
    RAISE NOTICE '';
    RAISE NOTICE '🎯 AUTOMATION FLOW RESTORED:';
    RAISE NOTICE '1. liveUpdateService.ts marks games as completed (your changes working)';
    RAISE NOTICE '2. handle_game_completion_only_trigger calculates base_points, margin_bonus';
    RAISE NOTICE '3. process_picks_on_completion_trigger calls calculate_pick_results()';
    RAISE NOTICE '4. calculate_pick_results() updates picks & anonymous_picks tables';
    RAISE NOTICE '';
    RAISE NOTICE '📋 ACTIVE TRIGGERS ON GAMES TABLE:';
    RAISE NOTICE '✅ handle_game_completion_only_trigger (BEFORE UPDATE - game scoring)';
    RAISE NOTICE '✅ process_picks_on_completion_trigger (AFTER UPDATE - pick processing)';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '  → Pick results calculated (win/loss/push, points_earned)';
    RAISE NOTICE '  → Both picks and anonymous_picks tables updated automatically';
    RAISE NOTICE '';
    RAISE NOTICE '🎉 Your requested flow is now working:';
    RAISE NOTICE '"update games to completed status, base_points and margin_bonus, then triggers updating picks table & anonymous_picks scoring"';
END;
$$;