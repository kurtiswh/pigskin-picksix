-- Fix leaderboard trigger functions - display_name field access issue
-- The triggers were trying to access user_record.display_name but only selecting display_name into the record

-- Drop and recreate the weekly leaderboard trigger function
...
-- Drop and recreate the season leaderboard trigger function
...
-- Add comments
COMMENT ON FUNCTION update_weekly_leaderboard_on_pick_change() IS 'Updates weekly_leaderboard table when picks are modified - fixes display_name field access issue';
COMMENT ON FUNCTION update_season_leaderboard_on_pick_change() IS 'Updates season_leaderboard table when picks are modified - fixes display_name field access issue';