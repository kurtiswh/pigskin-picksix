-- 178: Fix merge_users. (a) removed obsolete UPDATEs of season_leaderboard/
--   weekly_leaderboard (now non-updatable VIEWS derived from anonymous_picks);
--   (b) email-merge now clears is_primary_user_email (the column the
--   one-primary-per-user unique index uses) so moving a source email no longer
--   collides with the target's primary.
