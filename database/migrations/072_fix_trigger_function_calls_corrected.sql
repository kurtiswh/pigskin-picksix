...
-- CORRECTED VERSION - Fixed PostgreSQL syntax errors
...
-- Step 5: Test the fix with Nebraska game
...
-- Step 6: Test if the triggers now maintain correct values (FIXED SYNTAX)
-- Get one pick ID to trigger recalculation without LIMIT syntax error
DO $$
DECLARE
    test_pick_id UUID;
BEGIN
...
-- Step 7: Final verification - picks should remain correct
SELECT 
...
SELECT 
    ...
    'Triggers now call functions that use games.winner_against_spread' as fix_applied,
    'Cincinnati picks: WIN (20 pts), Nebraska picks: LOSS (0 pts)' as expected_result,
    'Triggers should maintain these correct values' as outcome;